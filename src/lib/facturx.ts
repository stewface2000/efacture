...
/**
 * Generate a professional Quote PDF (no Factur-X XML attached)
 */
export async function generateQuotePDF(
  ...
  page.drawText(quote.quoteNumber, {
    x: width - marginRight - font.widthOfTextAtSize(quote.quoteNumber, 12),
    y: height - 45,
    size: 12,
    font: fontBold,
    color: rgb(1, 1, 1),
  ...
  const dateStr = formatDate(quote.issueDate);
  page.drawText(`Date : ${dateStr}`, {
    x: width - marginRight - font.widthOfTextAtSize(`Date : ${dateStr}`, 9),
    y: height - 62,
    size: 9,
  ...
  const clientLines = [
    quote.clientName,
    quote.clientAddress,
    [quote.clientPostalCode, quote.clientCity].filter(Boolean).join(" "),
    quote.clientSiret ? `SIRET : ${quote.clientSiret}` : null,
    quote.clientVatNumber ? `TVA : ${quote.clientVatNumber}` : null,
  ].filter(Boolean) as string[];
  ...
  page.drawText("Total HT", { x: totalsX, y, size: 9, font, color: grey });
  page.drawText(`${quote.totalHT.toFixed(2)} €`, { x: totalsValX, y, size: 9, font, color: black });
  y -= 16;

  page.drawText("TVA", { x: totalsX, y, size: 9, font, color: grey });
  page.drawText(`${quote.totalVAT.toFixed(2)} €`, { x: totalsValX, y, size: 9, font, color: black });
  y -= 20;
  ...
  page.drawText("TOTAL TTC", { x: totalsX, y, size: 10, font: fontBold, color: rgb(1, 1, 1) });
  page.drawText(`${quote.totalTTC.toFixed(2)} €`, {
    x: totalsValX,
    y,
    size: 11,
    font: fontBold,
  ...
  // === VALIDITY DATE ===
  if (quote.validUntil) {
    page.drawText(`Ce devis est valable jusqu'au : ${formatDate(quote.validUntil)}`, {
      x: marginLeft,
      y,
      size: 9,
      font: fontBold,
  ...
  // Set PDF metadata (no Factur-X XML for quotes)
